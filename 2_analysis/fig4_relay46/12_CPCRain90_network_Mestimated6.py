...
ds1 = xr.open_dataset("2_analysis/fig4_relay46/Network_CPCRain90_2x2_lag_46810.nc")
networkRain = ds1.network6_weight
Size = np.shape(networkRain)
print(" networkRain = ",np.nanmin(networkRain),"  ",np.nanmax(networkRain))

##--##--##--##--    4D -> 2D    --##--##--##--##
networkRain_3D = np.zeros([Size[0],Size[1], Size[2]*Size[3]], dtype=np.float32)
for ilat in range(Size[2]):
    networkRain_3D[:,:,ilat*Size[3]:ilat*Size[3]+Size[3]] = networkRain[:,:,ilat,:]
# del networkRain
print(" networkRain 3D = ",np.nanmin(networkRain_3D),"  ",np.nanmax(networkRain_3D))

networkRain_2D = np.zeros([Size[0]*Size[1], Size[2]*Size[3]], dtype=np.float32)
for ilat in range(Size[0]):
    networkRain_2D[ilat*Size[1]:ilat*Size[1]+Size[1],:] = networkRain_3D[ilat,:,:]
del networkRain_3D
print(" networkRain 2D = ",np.nanmin(networkRain_2D),"  ",np.nanmax(networkRain_2D))
...
print(np.shape(networkRain_2D))
M20 = np.dot(network_2D, networkRain_2D)
...
M30 = np.dot(M20_notfinish, networkRain_2D)
del M20_notfinish

M40 = np.dot(M30_notfinish, networkRain_2D)
del M30_notfinish

M50 = np.dot(M40_notfinish, networkRain_2D)
del M40_notfinish
del network_2D, networkRain_2D
...
##--##--##--##--    to nc file    --##--##--##--##
M1_array0= xr.DataArray(data=networkRain, dims=['lat1', 'lon1', 'lat2', 'lon2'],
...
ds0.to_netcdf("2_analysis/fig4_relay46/Network_CPCRain90_Mestimated6.nc")
ds0.close()
